...
}), { getPrototypeOf: u, defineProperty: d, getOwnPropertyNames: f } = Object;
((e) => typeof l < "u" ? l : typeof Proxy < "u" ? new Proxy(e, { get: (e, t) => (typeof l < "u" ? l : e)[t] }) : e)(function(e) {
	if (typeof l < "u") return l.apply(this, arguments);
...
var p = class e {
	...
	stringArraySize(e) {
		let t = e.length * 4, n = e.reduce((e, t) => e + this.byteLength(t) + 1, 0);
		return {
			pointerArraySize: t,
			bufferSize: n,
			totalSize: t + n
	...
	writeStringArray(e, t, n, r) {
		let i = n, a = r;
		for (let n of t) e.setUint32(i, a, !0), i += 4, a += this.writeString(e, `${n}\x00`, a);
		return a - r;
	}
	writeString(e, t, n) {
		let r = this.encoder.encode(t);
		return new Uint8Array(e.buffer, n, r.length).set(r), r.length;
	}
	readString(e, t, n) {
		let r = new Uint8Array(e.buffer, t, n);
	...
	iovViews(t, n, r) {
		let i = [], a = n;
		for (let n = 0; n < r; n++) {
			let n = t.getUint32(a + e.iovec_t.bufferOffset, !0), r = t.getUint32(a + e.iovec_t.lengthOffset, !0);
			i.push(new Uint8Array(t.buffer, n, r)), a += e.iovec_t.size;
		}
		return i;
	}
	writeFilestat(e, t, n, r = 0n, i = 0n, a = 0n, o = 0n) {
		e.setBigUint64(t, 0n, !0), e.setBigUint64(t + 8, 0n, !0), e.setUint8(t + 16, n), e.setBigUint64(t + 24, 1n, !0), e.setBigUint64(t + 32, r, !0), e.setBigUint64(t + 40, i, !0), e.setBigUint64(t + 48, a, !0), e.setBigUint64(t + 56, o, !0);
	}
	writeFdstat(e, t, n, r, i, a) {
		e.setUint8(t, n), e.setUint16(t + 2, r, !0), e.setBigUint64(t + 8, i, !0), e.setBigUint64(t + 16, a, !0);
	}
}, m = class {
	code;
	constructor(e) {
...
function h(e, t, n) {
	...
	return {
		args_get: (e, i) => {
			let a = n();
			return t.writeStringArray(a, r, e, i), p.WASI_ESUCCESS;
		},
		args_sizes_get: (e, i) => {
			let a = n();
			a.setUint32(e, r.length, !0);
			let o = t.stringArraySize(r);
			return a.setUint32(i, o.bufferSize, !0), p.WASI_ESUCCESS;
		}
	};
}
function g(e, t, n) {
	return {
		clock_res_get: (e, t) => {
			let r;
			switch (e) {
				case p.WASI_CLOCK_MONOTONIC:
					r = 5e3;
					break;
				case p.WASI_CLOCK_REALTIME:
					r = 1e3;
					break;
				default: return p.WASI_ENOSYS;
			}
			return n().setUint32(t, r, !0), p.WASI_ESUCCESS;
		},
		clock_time_get: (e, t, r) => {
			...
			if (BigInt) {
				let e = BigInt(((e) => {
					let t = Math.trunc(e), n = BigInt(Math.round((e - t) * 1e6));
					return BigInt(t) * BigInt(1e6) + n;
				})(i));
				a.setBigUint64(r, e, !0);
			} else {
				let e = Date.now() * 1e6;
				a.setUint32(r, e & 65535, !0), a.setUint32(r + 4, e & 4294901760, !0);
			}
			return p.WASI_ESUCCESS;
		}
	};
}
function _(e, t, n) {
	return {
		environ_get: (r, i) => {
			...
			for (let n in e.env) {
				let r = e.env[n];
				s.setUint32(a, o, !0), a += 4, o += t.writeString(s, `${n}=${r}\x00`, o);
			}
			return p.WASI_ESUCCESS;
		},
		environ_sizes_get: (r, i) => {
			let a = n();
			return a.setUint32(r, Object.keys(e.env || {}).length, !0), a.setUint32(i, Object.entries(e.env || {}).reduce((e, [n, r]) => e + t.byteLength(n) + 1 + t.byteLength(r) + 1, 0), !0), p.WASI_ESUCCESS;
		}
	};
}
var v = class {
	...
	constructor(e, t) {
		this.handler = e, this.outputBuffers = t;
	}
	writev(e) {
		let t = e.reduce((e, t) => e + t.byteLength, 0), n = 0, r = new Uint8Array(t);
		for (let t of e) r.set(t, n), n += t.byteLength;
		if (this.outputBuffers) this.handler(r);
		else {
			let e = this.decoder.decode(r);
			this.handler(e);
		}
		return r.length;
	}
	...
	consumePending(e, t) {
		...
		let n = e.slice(0, t);
		return this.pending = e.slice(t), n;
	}
	readv(e) {
		let t = 0;
		for (let n of e) {
			let e = n.byteLength;
			if (this.pending) {
				let r = this.consumePending(this.pending, e);
				n.set(r, 0), e -= r.byteLength, t += r.byteLength;
			}
			for (; e > 0;) {
				let r = this.consume(), i;
				if (i = r instanceof Uint8Array ? r : this.encoder.encode(r), i.length === 0) return t;
				i.length > e ? (n.set(i.slice(0, e), n.byteLength - e), this.pending = i.slice(e), t += e, e = 0) : (n.set(i, n.byteLength - e), t += i.length, e -= i.length);
			}
		}
		return t;
	}
...
function b(e = {}) {
	let t = e.outputBuffers || !1;
	return [
		new y(e.stdin || (() => "")),
		new v(e.stdout || console.log, t),
		new v(e.stderr || console.error, t)
	];
}
var x = class {
	root;
	...
	constructor(e) {
		if (this.root = {
			type: "dir",
			entries: {}
		}, this.ensureDir("/dev"), this.setNode("/dev/null", {
			type: "character",
			kind: "devnull"
		}), e) for (let t of Object.keys(e)) this.ensureDir(t), this.preopenPaths.push(t);
		else this.preopenPaths.push("/");
	}
	removeFile(e) {
		let t = this.normalizePath(e).split("/").filter((e) => e.length > 0), n = t.pop(), r = `/${t.join("/")}`, i = this.ensureDir(r);
		n && delete i.entries[n];
	}
	addFile(e, t) {
		if (typeof t == "string") {
	...
	createFile(e, t) {
		...
		return this.setNode(e, n), n;
	}
	setNode(e, t) {
		let n = this.normalizePath(e).split("/").filter((e) => e.length > 0);
		if (n.length === 0) {
			if (t.type !== "dir") throw Error("Root must be a directory");
			this.root = t;
			return;
		}
		let r = n.pop(), i = `/${n.join("/")}`, a = this.ensureDir(i);
		r && (a.entries[r] = t);
	}
	getDevNull() {
	...
	lookup(e) {
		...
		let n = t.split("/").filter((e) => e.length > 0), r = this.root;
		for (let e of n) if (r.type !== "dir" || (r = r.entries[e], !r)) return null;
		return r;
	}
	resolve(e, t) {
		let n = this.normalizePath(t).split("/").filter((e) => e.length > 0), r = e;
		for (let e of n) if (e !== ".") {
			if (e === "..") {
				r = this.root;
				continue;
			}
			if (r.type !== "dir" || (r = r.entries[e], !r)) return null;
		}
		return r;
	}
	ensureDir(e) {
		let t = this.normalizePath(e).split("/").filter((e) => e.length > 0), n = this.root;
		for (let e of t) {
			n.entries[e] || (n.entries[e] = {
				type: "dir",
				entries: {}
			});
			let t = n.entries[e];
			if (t.type !== "dir") throw Error(`"${e}" is not a directory`);
			n = t;
		}
		return n;
	}
	createFileIn(e, t) {
		let n = this.normalizePath(t).split("/").filter((e) => e.length > 0);
		if (n.length === 0) throw Error("Cannot create a file with an empty name");
		let r = n.pop();
		if (!r) throw Error("Cannot create a file with an empty name");
		let i = e;
		for (let e of n) {
			i.entries[e] || (i.entries[e] = {
				type: "dir",
				entries: {}
			});
			let t = i.entries[e];
		...
		return i.entries[r] = a, a;
	}
	normalizePath(e) {
		if (!e) return "/";
		let t = (e.startsWith("/") ? e : `/${e}`).replace(/\/+/g, "/");
		return t === "/" ? t : t.replace(/\/+$/, "");
	}
};
function S(e = {}) {
	return (t, n, r) => {
		let i = e.withFileSystem || new x(t.preopens), a = {};
		function o(e) {
			if (e.type === "file" && e.content instanceof Blob) {
				let t = BigInt(e.content.lastModified ?? Date.now()) * 1000000n;
				return {
					atim: t,
					mtim: t,
					ctim: t
		...
		let s = 3;
		for (let e of i.getPreopenPaths()) {
			let t = i.lookup(e);
			t && t.type === "dir" && (a[s] = {
				node: t,
				position: 0,
		...
		function u(e) {
			return e.content instanceof Blob ? e.content.size : e.content.byteLength;
		}
		return {
			fd_read: async (e, t, i, a) => {
				let o = r(), s = n.iovViews(o, t, i), c = l(e);
				if (!c) return p.WASI_ERRNO_BADF;
				if (c.node.type === "character" && c.node.kind === "stdio") {
					let e = c.node.entry.readv(s);
					return o.setUint32(a, e, !0), p.WASI_ESUCCESS;
				}
				if (c.node.type === "dir") return p.WASI_ERRNO_ISDIR;
				if (c.node.type === "character" && c.node.kind === "devnull") return o.setUint32(a, 0, !0), p.WASI_ESUCCESS;
				let d = c.node, f = d.content, m = u(d) - c.position, h = 0;
				if (m <= 0) return o.setUint32(a, 0, !0), p.WASI_ESUCCESS;
				if (d.content instanceof Blob) {
					let e = d.content;
					for (let t of s) {
						if (c.position >= e.size) break;
						let n = Math.min(t.byteLength, e.size - c.position);
						if (n <= 0) break;
						let r = await e.slice(c.position, c.position + n).arrayBuffer();
						t.set(new Uint8Array(r)), h += r.byteLength, c.position += r.byteLength;
					}
				} else if (ArrayBuffer.isView(f)) for (let e of s) {
					if (c.position >= f.byteLength) break;
					let t = Math.min(e.byteLength, f.byteLength - c.position);
					if (t <= 0) break;
					e.set(f.slice(c.position, c.position + t)), h += t, c.position += t;
				}
				return o.setUint32(a, h, !0), p.WASI_ESUCCESS;
			},
			fd_write: (e, t, i, a) => {
				let o = r(), s = n.iovViews(o, t, i), c = l(e);
				if (!c) return p.WASI_ERRNO_BADF;
				let d = 0;
				if (c.node.type === "character" && c.node.kind === "stdio") {
					let e = c.node.entry.writev(s);
					return o.setUint32(a, e, !0), p.WASI_ESUCCESS;
				}
				if (c.node.type === "dir") return p.WASI_ERRNO_ISDIR;
				if (c.node.type === "character" && c.node.kind === "devnull") {
					let e = s.reduce((e, t) => e + t.byteLength, 0);
					return o.setUint32(a, e, !0), p.WASI_ESUCCESS;
				}
				if (c.node.content instanceof Blob) return p.WASI_ERRNO_INVAL;
				let f = c.position, m = s.reduce((e, t) => e + t.byteLength, 0), h = f + m, g;
				h > u(c.node) ? (g = new Uint8Array(h), g.set(c.node.content, 0)) : g = c.node.content;
				for (let e of s) g.set(e, f), f += e.byteLength, d += e.byteLength;
				return c.node.content = g, c.position = f, o.setUint32(a, d, !0), p.WASI_ESUCCESS;
			},
			fd_close: (e) => {
				let t = l(e);
				return t ? t.node.type === "character" && t.node.kind === "stdio" ? (t.node.entry.close(), p.WASI_ESUCCESS) : (delete a[e], p.WASI_ESUCCESS) : p.WASI_ERRNO_BADF;
			},
			fd_seek: (e, t, n, i) => {
				let a = r(), o = l(e);
				if (!o) return p.WASI_ERRNO_BADF;
				if (o.node.type === "dir") return p.WASI_ERRNO_ISDIR;
				if (o.node.type === "character") return p.WASI_ERRNO_IO;
				let s = u(o.node), c;
				switch (n) {
					case 0:
						c = Number(t);
						break;
					case 1:
						c = o.position + Number(t);
						break;
					case 2:
						c = s + Number(t);
						break;
					default: return p.WASI_ERRNO_INVAL;
				}
				return c < 0 ? p.WASI_ERRNO_INVAL : (o.position = c, a.setBigUint64(i, BigInt(c), !0), p.WASI_ESUCCESS);
			},
			fd_tell: (e, t) => {
				let n = r(), i = l(e);
				return i ? i.node.type === "dir" || i.node.type === "character" ? p.WASI_ERRNO_IO : (n.setBigUint64(t, BigInt(i.position), !0), p.WASI_ESUCCESS) : p.WASI_ERRNO_BADF;
			},
			fd_fdstat_get: (e, t) => {
				let i = r(), a = l(e);
				if (!a) return p.WASI_ERRNO_BADF;
				let o;
				switch (a.node.type) {
					case "character":
						o = p.WASI_FILETYPE_CHARACTER_DEVICE;
						break;
					case "dir":
						o = p.WASI_FILETYPE_DIRECTORY;
						break;
					case "file":
						o = p.WASI_FILETYPE_REGULAR_FILE;
						break;
				}
				let s = 536870911n;
				return n.writeFdstat(i, t, o, 0, s, s), p.WASI_ESUCCESS;
			},
			fd_filestat_get: (e, t) => {
				let i = r(), a = l(e);
				if (!a) return p.WASI_ERRNO_BADF;
				...
				switch (a.node.type) {
					case "character":
						s = p.WASI_FILETYPE_CHARACTER_DEVICE;
						break;
					case "dir":
						s = p.WASI_FILETYPE_DIRECTORY;
						break;
					case "file":
						s = p.WASI_FILETYPE_REGULAR_FILE, c = u(a.node);
						break;
				}
				let { atim: d, mtim: f, ctim: m } = o(a.node);
				return n.writeFilestat(i, t, s, BigInt(c), d, f, m), p.WASI_ESUCCESS;
			},
			fd_prestat_get: (e, t) => {
				let n = r();
				if (e < 3) return p.WASI_ERRNO_BADF;
				let i = l(e);
				if (!i || !i.isPreopen) return p.WASI_ERRNO_BADF;
				n.setUint8(t, 0);
				let a = i.preopenPath || "";
				return n.setUint32(t + 4, a.length, !0), p.WASI_ESUCCESS;
			},
			fd_prestat_dir_name: (e, t, n) => {
				if (e < 3) return p.WASI_ERRNO_BADF;
				let i = l(e);
				if (!i || !i.isPreopen) return p.WASI_ERRNO_BADF;
				let a = i.preopenPath || "", o = r(), s = Math.min(a.length, n);
				for (let e = 0; e < s; e++) o.setUint8(t + e, a.charCodeAt(e));
				return p.WASI_ESUCCESS;
			},
			fd_open: (e, t, o, u, d, f, m, h) => {
				let g = r();
				if (e < 3) return p.WASI_ERRNO_NOTDIR;
				let _ = l(e);
				if (!_ || _.node.type !== "dir") return p.WASI_ERRNO_NOTDIR;
				let v = n.readString(g, t, o), y = (_.path.endsWith("/") ? _.path : `${_.path}/`) + v, b = c(y);
				if (b) return g.setUint32(h, b.fd, !0), p.WASI_ESUCCESS;
				let x = i.resolve(_.node, v);
				if (x) {
					if (u & 2) return p.WASI_ERRNO_EXIST;
					if (u & 4) {
						if (x.type !== "file") return p.WASI_ERRNO_INVAL;
						x.content = new Uint8Array();
					}
				} else {
					if (!(u & 1)) return p.WASI_ERRNO_NOENT;
					x = i.createFileIn(_.node, v);
				}
				return a[s] = {
					node: x,
					position: 0,
					...
					path: y,
					fd: s
				}, g.setUint32(h, s, !0), s++, p.WASI_ESUCCESS;
			},
			path_open: (e, t, o, u, d, f, m, h, g) => {
				let _ = r();
				if (e < 3) return p.WASI_ERRNO_NOTDIR;
				let v = l(e);
				if (!v || v.node.type !== "dir") return p.WASI_ERRNO_NOTDIR;
				let y = n.readString(_, o, u), b = (v.path.endsWith("/") ? v.path : `${v.path}/`) + y, x = c(b);
				if (x) return _.setUint32(g, x.fd, !0), p.WASI_ESUCCESS;
				let S = i.resolve(v.node, y);
				if (S) {
					if (d & 2) return p.WASI_ERRNO_EXIST;
					if (d & 4) {
						if (S.type !== "file") return p.WASI_ERRNO_INVAL;
						S.content = new Uint8Array();
					}
				} else {
					if (!(d & 1)) return p.WASI_ERRNO_NOENT;
					S = i.createFileIn(v.node, y);
				}
				return a[s] = {
					node: S,
					position: 0,
					...
					path: b,
					fd: s
				}, _.setUint32(g, s, !0), s++, p.WASI_ESUCCESS;
			},
			path_filestat_get: (e, t, a, s, c) => {
				let d = r(), f = l(e);
				if (!f) return p.WASI_ERRNO_BADF;
				if (f.node.type !== "dir") return p.WASI_ERRNO_NOTDIR;
				let m = n.readString(d, a, s), h = f.path, g = h.endsWith("/") ? h + m : `${h}/${m}`, _ = i.lookup(g);
				if (!_) return p.WASI_ERRNO_NOENT;
				if (_.type === "character" && _.kind === "stdio") return p.WASI_ERRNO_INVAL;
				let v, y = 0;
				_.type === "dir" ? v = p.WASI_FILETYPE_DIRECTORY : _.type === "character" && _.kind === "devnull" ? v = p.WASI_FILETYPE_CHARACTER_DEVICE : (v = p.WASI_FILETYPE_REGULAR_FILE, y = u(_));
				let { atim: b, mtim: x, ctim: S } = o(_);
				return n.writeFilestat(d, c, v, BigInt(y), b, x, S), p.WASI_ESUCCESS;
			}
		};
	};
}
function C(e, t, n) {
	return {
		...
		proc_raise: (e) => p.WASI_ESUCCESS
	};
}
function w(e, t, n) {
	return { random_get: (e, t) => {
		let r = n(), i = new Uint8Array(r.buffer, e, t);
		return crypto.getRandomValues(i), p.WASI_ESUCCESS;
	} };
}
var T = class {
	wasiImport;
	...
	abi;
	constructor(e) {
		if (this.wasiImport = {}, this.abi = new p(), e?.features) {
			let t = {};
			for (let n of e.features) {
				let r = n.name || "Unknown feature", i = n(e, this.abi, this.view.bind(this));
				for (let e in i) {
					if (e in this.wasiImport) {
						let n = t[e] || "Unknown feature";
						throw Error(`Import conflict: Function '${e}' is already provided by '${n}' and is being redefined by '${r}'`);
					}
					t[e] = r;
				}
				this.wasiImport = {
		...
		for (let e of p.IMPORT_FUNCTIONS) e in this.wasiImport || (this.wasiImport[e] = () => p.WASI_ENOSYS);
	}
	get exports() {
	...
	async initialize(e) {
		...
		if (this.isStarted = !0, this.instance = e, !this.instance.exports._initialize) throw Error("instance.exports._initialize is undefined");
	...
	async start(e) {
		if (this.isStarted) throw Error("wasi.start() or wasi.initialize() has already been called");
		if (this.isStarted = !0, this.instance = e, !this.instance.exports._start) throw Error("instance.exports._start is undefined");
		if (typeof this.instance.exports._start != "function") throw Error("instance.exports._start is not a function");
		try {
			return await this.instance.exports._start(), p.WASI_ESUCCESS;
		} catch (e) {
			if (e instanceof m) return e.code;
			throw e;
		}
	}
}, E = /* @__PURE__ */ new WeakMap(), D = new Set(["free", "malloc"]);
function O(e) {
	return !!e && (typeof e == "object" || typeof e == "function") && typeof e.then == "function";
}
function k(e, t) {
	return new Proxy(e, { get: (e, n) => t(e[n]) });
}
var A = class {
	value = void 0;
	exports = null;
	unwrappedExports;
	constructor(e) {
		this.unwrappedExports = new Set([...D, ...e?.unwrappedExports ?? []]);
	}
	getState() {
	...
	wrapImportFn(e) {
		return (...t) => {
			...
			if (!O(n)) return n;
			if (!this.exports) throw Error("Exports not initialized");
			this.exports.asyncify_start_unwind(16), this.value = n;
		};
	}
	wrapModuleImports(e) {
		return k(e, (e) => typeof e == "function" ? this.wrapImportFn(e) : e);
	}
	wrapImports(e) {
		if (e !== void 0) return k(e, (e = Object.create(null)) => this.wrapModuleImports(e));
	}
	wrapExportFn(e) {
		let t = E.get(e);
		return t === void 0 ? (t = async (...t) => {
			this.assertNoneState();
			let n = e(...t);
			for (; this.getState() === 1;) {
				if (!this.exports) throw Error("Exports not initialized");
				this.exports.asyncify_stop_unwind(), this.value = await this.value, this.assertNoneState(), this.exports.asyncify_start_rewind(16), n = e(...t);
			}
	...
	wrapExports(e) {
		...
		for (let n in e) {
			let r = e[n];
			typeof r == "function" && !n.startsWith("asyncify_") && !this.unwrappedExports.has(n) && (r = this.wrapExportFn(r)), Object.defineProperty(t, n, {
				enumerable: !0,
				value: r
			});
		}
...
async function B(e) {
	...
	let t;
	if (z()) t = await (await (e ?? fetch)(N)).arrayBuffer();
	else {
		let e = new URL(N, import.meta.url).pathname;
		if (typeof Deno < "u") t = (await Deno.readFile(e)).buffer;
		else if (typeof Bun < "u") t = await Bun.file(e).arrayBuffer();
		else {
			let { readFile: n } = await import("./assets/__vite-browser-external-D3qcbAIu.js").then((e) => /* @__PURE__ */ c(e.default, 1));
			t = (await n(e)).buffer;
		}
	}
	return R = new WeakRef(t), t;
}
function V(e) {
	return [
		"undef",
...
var U = class e {
	...
	toInt() {
		...
		try {
			if (!this.exports.zeroperl_to_int(this.ptr, e)) throw new F("Failed to convert value to int");
			return new DataView(this.exports.memory.buffer).getInt32(e, !0);
		} finally {
			this.exports.free(e);
		}
	}
	toDouble() {
		...
		try {
			if (!this.exports.zeroperl_to_double(this.ptr, e)) throw new F("Failed to convert value to double");
			return new DataView(this.exports.memory.buffer).getFloat64(e, !0);
		} finally {
			this.exports.free(e);
		}
	}
	toString() {
		...
		try {
			...
			if (t === 0) return "";
			let n = new DataView(this.exports.memory.buffer).getUint32(e, !0);
			return I.decode(new Uint8Array(this.exports.memory.buffer, t, n));
		} finally {
			this.exports.free(e);
		}
	}
	...
	getType() {
		return this.checkDisposed(), V(this.exports.zeroperl_get_type(this.ptr));
	}
	project() {
	...
	createRef() {
		this.checkDisposed();
		let t = this.exports.zeroperl_new_ref(this.ptr);
		if (t === 0) throw new F("Failed to create reference");
		return new e(t, this.exports);
	}
	deref() {
		this.checkDisposed();
		let t = this.exports.zeroperl_deref(this.ptr);
		if (t === 0) throw new F("Failed to dereference value");
		return new e(t, this.exports);
	}
	incref() {
		this.checkDisposed(), this.exports.zeroperl_incref(this.ptr);
	}
	decref() {
		this.checkDisposed(), this.exports.zeroperl_decref(this.ptr);
	}
	dispose() {
		this.disposed ||= (this.exports.zeroperl_value_free(this.ptr), !0);
	}
	checkDisposed() {
		if (this.disposed) throw new F("PerlValue has been disposed");
	}
}, W = class e {
	ptr;
	exports;
	...
	disposed = !1;
	constructor(e, t, n) {
		this.ptr = e, this.exports = t, this.perl = n;
	}
	getPtr() {
		return this.checkDisposed(), this.ptr;
	}
	push(e) {
		...
		try {
			this.exports.zeroperl_array_push(this.ptr, t.getPtr());
		} finally {
			e instanceof U || t.dispose();
		}
	}
	pop() {
		this.checkDisposed();
		let e = this.exports.zeroperl_array_pop(this.ptr);
		return e === 0 ? null : new U(e, this.exports);
	}
	get(e) {
		this.checkDisposed();
		let t = this.exports.zeroperl_array_get(this.ptr, e);
		return t === 0 ? null : new U(t, this.exports);
	}
	set(e, t) {
		this.checkDisposed();
		let n = this.perl.toPerlValue(t);
		try {
			if (!this.exports.zeroperl_array_set(this.ptr, e, n.getPtr())) throw new F(`Failed to set array element at index ${e}`);
		} finally {
			t instanceof U || n.dispose();
		}
	}
	getLength() {
	...
	clear() {
		this.checkDisposed(), this.exports.zeroperl_array_clear(this.ptr);
	}
	toValue() {
		this.checkDisposed();
		let e = this.exports.zeroperl_array_to_value(this.ptr);
		if (e === 0) throw new F("Failed to convert array to value");
		return new U(e, this.exports);
	}
	project() {
		this.checkDisposed();
		...
		for (let n = 0; n < e; n++) {
			let e = this.get(n);
			e ? (t.push(e.project()), e.dispose()) : t.push(null);
		}
		return t;
	}
	static fromValue(t, n) {
		let r = t.exports, i = r.zeroperl_value_to_array(t.getPtr());
		return i === 0 ? null : new e(i, r, n);
	}
	*[Symbol.iterator]() {
	...
	dispose() {
		this.disposed ||= (this.exports.zeroperl_array_free(this.ptr), !0);
	}
	checkDisposed() {
		if (this.disposed) throw new F("PerlArray has been disposed");
	}
}, G = class e {
	ptr;
	exports;
	...
	set(e, t) {
		this.checkDisposed();
		let n = this.perl.toPerlValue(t), r = this.writeCString(e);
		try {
			if (!this.exports.zeroperl_hash_set(this.ptr, r, n.getPtr())) throw new F(`Failed to set hash key '${e}'`);
		} finally {
			this.exports.free(r), !(t instanceof U) && n.dispose();
		}
	}
	get(e) {
		this.checkDisposed();
		...
		try {
			let e = this.exports.zeroperl_hash_get(this.ptr, t);
			return e === 0 ? null : new U(e, this.exports);
		} finally {
			this.exports.free(t);
		}
	...
	delete(e) {
		...
		let t = this.writeCString(e);
		try {
			return this.exports.zeroperl_hash_delete(this.ptr, t) !== 0;
		} finally {
			this.exports.free(t);
		}
	}
	clear() {
		this.checkDisposed(), this.exports.zeroperl_hash_clear(this.ptr);
	}
	toValue() {
		this.checkDisposed();
		let e = this.exports.zeroperl_hash_to_value(this.ptr);
		if (e === 0) throw new F("Failed to convert hash to value");
		return new U(e, this.exports);
	}
	project() {
		this.checkDisposed();
		let e = {};
		for (let [t, n] of this.entries()) e[t] = n.project(), n.dispose();
		return e;
	}
	static fromValue(t, n) {
		let r = t.exports, i = r.zeroperl_value_to_hash(t.getPtr());
		return i === 0 ? null : new e(i, r, n);
	}
	*entries() {
		this.checkDisposed();
		let e = this.exports.zeroperl_hash_iter_new(this.ptr);
		if (e === 0) throw new F("Failed to create hash iterator");
		let t = this.exports.malloc(4), n = this.exports.malloc(4);
		try {
			for (; this.exports.zeroperl_hash_iter_next(e, t, n);) {
				let e = new DataView(this.exports.memory.buffer), r = e.getUint32(t, !0), i = e.getUint32(n, !0);
				yield [this.readCString(r), new U(i, this.exports)];
			}
		} finally {
			this.exports.free(t), this.exports.free(n), this.exports.zeroperl_hash_iter_free(e);
		}
	}
	*keys() {
	...
	*values() {
	...
	dispose() {
		this.disposed ||= (this.exports.zeroperl_hash_free(this.ptr), !0);
	}
	writeCString(e) {
		let t = L.encode(`${e}\x00`), n = this.exports.malloc(t.length);
		return new Uint8Array(this.exports.memory.buffer).set(t, n), n;
	}
	readCString(e) {
		if (e === 0) return "";
		let t = new Uint8Array(this.exports.memory.buffer), n = 0;
		for (; t[e + n] !== 0;) n++;
		return I.decode(t.subarray(e, e + n));
	}
	checkDisposed() {
		if (this.disposed) throw new F("PerlHash has been disposed");
	}
}, ee = class e {
	wasi;
	isDisposed = !1;
	...
	static async create(t = {}) {
		let n = await B(t.fetch), r = t.fileSystem || new x({ "/": "" }), i = new T({
			env: t.env || {},
			args: ["zeroperl"],
			features: [
				_,
		...
		}), a = new e(i), { instance: o } = await M(n, {
			wasi_snapshot_preview1: i.wasiImport,
			env: { call_host_function: async (e, t, n) => a.handleHostCall(e, t, n) }
		}, { unwrappedExports: P });
		await i.initialize(o);
		let s = await a.exports.zeroperl_init();
		if (s !== 0) throw new F("Failed to initialize Perl interpreter", s, a.getLastError());
		return a;
	}
	async handleHostCall(e, t, n) {
		let r = this.hostFunctions.get(e);
		if (!r) return this.setHostError(`Host function ${e} not found`), 0;
		try {
			let e = [];
			if (t > 0) {
				let r = new DataView(this.exports.memory.buffer);
				for (let i = 0; i < t; i++) {
					let t = r.getUint32(n + i * 4, !0);
					t !== 0 && e.push(new U(t, this.exports));
				}
			}
			let i = await r(...e);
			if (i instanceof U) return i.getPtr();
			let a = this.exports.zeroperl_new_undef();
			return a === 0 ? (this.setHostError("Failed to allocate return value"), 0) : a;
		} catch (e) {
			return this.setHostError(e instanceof Error ? e.message : String(e)), 0;
		}
	}
	setHostError(e) {
		let t = this.writeCString(e);
		t && (this.exports.zeroperl_set_host_error(t), this.exports.free(t));
	}
	createInt(e) {
		this.checkDisposed();
		let t = this.exports.zeroperl_new_int(Math.floor(e));
		if (t === 0) throw new F("Failed to create integer value");
		return new U(t, this.exports);
	}
	createUInt(e) {
		this.checkDisposed();
		let t = this.exports.zeroperl_new_uint(Math.floor(Math.abs(e)));
		if (t === 0) throw new F("Failed to create unsigned integer value");
		return new U(t, this.exports);
	}
	createDouble(e) {
		this.checkDisposed();
		let t = this.exports.zeroperl_new_double(e);
		if (t === 0) throw new F("Failed to create double value");
		return new U(t, this.exports);
	}
	createString(e) {
		this.checkDisposed();
		let t = L.encode(e), n = this.exports.malloc(t.length);
		new Uint8Array(this.exports.memory.buffer).set(t, n);
		try {
			let e = this.exports.zeroperl_new_string(n, t.length);
			if (e === 0) throw new F("Failed to create string value");
			return new U(e, this.exports);
		} finally {
			this.exports.free(n);
		}
	...
	createBool(e) {
		this.checkDisposed();
		let t = this.exports.zeroperl_new_bool(+!!e);
		if (t === 0) throw new F("Failed to create boolean value");
		return new U(t, this.exports);
	}
	createUndef() {
		this.checkDisposed();
		let e = this.exports.zeroperl_new_undef();
		if (e === 0) throw new F("Failed to create undef value");
		return new U(e, this.exports);
	}
	createArray(e) {
		this.checkDisposed();
		let t = this.exports.zeroperl_new_array();
		if (t === 0) throw new F("Failed to create array");
		let n = new W(t, this.exports, this);
		if (e) for (let t of e) n.push(t);
		return n;
	}
	createHash(e) {
		this.checkDisposed();
		let t = this.exports.zeroperl_new_hash();
		if (t === 0) throw new F("Failed to create hash");
		let n = new G(t, this.exports, this);
		if (e) for (let [t, r] of Object.entries(e)) n.set(t, r);
		return n;
	}
	toPerlValue(e) {
		if (e instanceof U) return e;
		if (e == null) return this.createUndef();
		if (typeof e == "boolean") return this.createBool(e);
		if (typeof e == "number") return Number.isInteger(e) ? this.createInt(e) : this.createDouble(e);
		if (typeof e == "string") return this.createString(e);
		if (Array.isArray(e)) {
			let t = this.createArray(e), n = t.toValue();
			return t.dispose(), n;
		}
		if (typeof e == "object") {
			let t = this.createHash(e), n = t.toValue();
			return t.dispose(), n;
		}
		throw new F(`Cannot convert value of type ${typeof e} to PerlValue`);
	}
	getVariable(e) {
		this.checkDisposed();
		let t = this.writeCString(e);
		try {
			let e = this.exports.zeroperl_get_var(t);
			return e === 0 ? null : new U(e, this.exports);
		} finally {
			this.exports.free(t);
		}
	...
	getArrayVariable(e) {
		...
		let t = this.writeCString(e);
		try {
			let e = this.exports.zeroperl_get_array_var(t);
			return e === 0 ? null : new W(e, this.exports, this);
		} finally {
			this.exports.free(t);
		}
	}
	getHashVariable(e) {
		...
		let t = this.writeCString(e);
		try {
			let e = this.exports.zeroperl_get_hash_var(t);
			return e === 0 ? null : new G(e, this.exports, this);
		} finally {
			this.exports.free(t);
		}
	}
	setVariable(e, t) {
		this.checkDisposed();
		let n = this.toPerlValue(t), r = this.writeCString(e);
		try {
			if (!this.exports.zeroperl_set_var(r, n.getPtr())) throw new F(`Failed to set variable '${e}'`);
		} finally {
			this.exports.free(r), !(t instanceof U) && n.dispose();
		}
	}
	registerFunction(e, t) {
		...
		this.hostFunctions.set(n, t);
		let r = this.writeCString(e);
		try {
			this.exports.zeroperl_register_function(n, r);
		} finally {
			this.exports.free(r);
		}
	}
	registerMethod(e, t, n) {
		this.checkDisposed();
		let r = this.nextFuncId++;
		this.hostFunctions.set(r, n);
		let i = this.writeCString(e), a = this.writeCString(t);
		try {
			this.exports.zeroperl_register_method(r, i, a);
		} finally {
			this.exports.free(i), this.exports.free(a);
		}
	}
	async call(e, t = [], n = "scalar") {
		this.checkDisposed();
		let r = this.writeCString(e), i = H(n), a = 0;
		if (t.length > 0) {
			a = this.exports.malloc(t.length * 4);
			let e = new DataView(this.exports.memory.buffer);
			for (let n = 0; n < t.length; n++) {
				let r = t[n];
				if (!r) throw new F(`Argument at index ${n} is undefined`);
				e.setUint32(a + n * 4, r.getPtr(), !0);
			}
		}
		try {
			let e = await this.exports.zeroperl_call(r, i, t.length, a);
			if (e === 0) return n === "void" ? void 0 : n === "scalar" ? null : [];
			let o = new DataView(this.exports.memory.buffer), s = o.getInt32(e, !0), c = [];
			for (let t = 0; t < s; t++) {
				let n = this.exports.zeroperl_result_get(e, t);
				n !== 0 && c.push(new U(n, this.exports));
			}
			let l = o.getUint32(e + 4, !0);
			if (l !== 0 && this.exports.free(l), this.exports.free(e), n === "void") {
				for (let e of c) e.dispose();
				return;
			}
			return n === "scalar" ? c[0] ?? null : c;
		} catch (e) {
			if (e instanceof m) return n === "void" ? void 0 : n === "scalar" ? null : [];
			throw e;
		} finally {
			this.exports.free(r), a !== 0 && this.exports.free(a);
		}
	}
	async eval(e, t = []) {
		this.checkDisposed();
		let n = this.writeCString(e), r = 0, i = [];
		if (t.length > 0) {
			let e = this.writeStringArray(t);
			r = e.argv, i = e.buffers;
		}
		try {
			let e = await this.exports.zeroperl_eval(n, H("scalar"), t.length, r);
			return e === 0 ? {
				success: !0,
				exitCode: 0
			} : {
			...
			if (e instanceof m) return e.code === 0 ? {
				success: !0,
				exitCode: 0
			} : {
				success: !1,
			...
			this.exports.free(n), i.length > 0 && this.freeStringArray(r, i);
		}
	}
	async runFile(e, t = []) {
		this.checkDisposed();
		...
		try {
			let e = await this.exports.zeroperl_run_file(n, t.length, r);
			return e === 0 ? {
				success: !0,
				exitCode: 0
			} : {
	...
	async reset() {
		...
		if (e !== 0) throw new F("Failed to reset Perl interpreter", e, this.getLastError());
	}
	flush() {
		if (this.checkDisposed(), this.exports.zeroperl_flush() !== 0) throw new F("Failed to flush output buffers");
	}
	getLastError() {
	...
	dispose() {
		this.isDisposed || (this.exports.zeroperl_free_interpreter(), this.isDisposed = !0, this.hostFunctions.clear());
	}
	shutdown() {
		this.isDisposed || (this.exports.zeroperl_shutdown(), this.isDisposed = !0, this.hostFunctions.clear());
	}
	writeCString(e) {
		if (!e) return 0;
		let t = L.encode(`${e}\x00`), n = this.exports.malloc(t.length);
	...
	writeStringArray(e) {
		let t = [], n = this.exports.malloc(e.length * 4), r = new DataView(this.exports.memory.buffer);
		for (let i = 0; i < e.length; i++) {
			let a = e[i];
			if (a === void 0) throw new F(`Argument at index ${i} is undefined`);
			let o = this.writeCString(a);
			t.push(o), r.setUint32(n + i * 4, o, !0);
		}
		return {
			argv: n,
			buffers: t
...
`), this;
	...
	static isMultiline(e) {
		let t = 0;
		for (let n = 0; n < e.length; n++) (e[n] === "\n" || e[n] === "\r" && (n === e.length - 1 || e[n + 1] !== "\n")) && t++;
		return t > 0;
	}
}, q = null, J = null, Y = new K(), X = new K(), Z = new TextDecoder();
async function Q(e) {
	let t = q?.deref(), n = J?.deref();
	return t && n ? {
		perl: t,
		fileSystem: n
	} : (n = new x({ "/": "" }), n.addFile("/exiftool", te), t = await ee.create({
		fileSystem: n,
		stdout: (e) => {
			let t = typeof e == "string" ? e : Z.decode(e);
			Y.append(t);
		},
		stderr: (e) => {
			let t = typeof e == "string" ? e : Z.decode(e);
		...
		fetch: e
	}), q = new WeakRef(t), J = new WeakRef(n), {
		perl: t,
		fileSystem: n
	});
}
...
function ne(e) {
	return Object.entries(e).flatMap(([e, t]) => Array.isArray(t) ? t.map((t) => `-${e}=${t}`) : [`-${e}=${t}`]);
}
async function re(e, t = {}) {
	let { perl: n, fileSystem: r } = await Q(t.fetch), i = [];
	Y.clear(), X.clear(), await n.reset();
	try {
		let a = `/${e.name}`;
		e instanceof File ? r.addFile(a, e) : r.addFile(a, e.data), i.push(a);
		let o = [...t.args || []];
		if (t.config) {
			let e = `/${t.config.name}`;
			t.config instanceof File ? r.addFile(e, t.config) : r.addFile(e, t.config.data), i.push(e), o.push("-config", e);
		}
		o.push(a);
		let s = await n.runFile("/exiftool", o);
		n.flush();
		let c = X.toString(), l = Y.toString();
		if (!s.success || s.exitCode !== 0) return {
			success: !1,
			data: void 0,
			error: n.getLastError() || c || "Unknown error",
			exitCode: s.exitCode
		};
		if (c && c.trim()) return {
			success: !1,
		...
		let u;
		return u = t.transform ? t.transform(l) : l, {
			success: !0,
			data: u,
			exitCode: 0
		};
...
async function ie(e, t, n = {}) {
	let { perl: r, fileSystem: i } = await Q(n.fetch), a = [];
	Y.clear(), X.clear(), await r.reset();
	try {
		let o = `/${e.name}`;
		e instanceof File ? i.addFile(o, e) : i.addFile(o, e.data), a.push(o);
		let s = [...n.args || []];
		if (n.config) {
			let e = `/${n.config.name}`;
			n.config instanceof File ? i.addFile(e, n.config) : i.addFile(e, n.config.data), a.push(e), s.push("-config", e);
		}
		s.push(...ne(t));
		let c = `/${crypto.randomUUID().replace(/-/g, "")}.tmp`;
		a.push(c), s.push("-o", c), s.push(o);
		let l = await r.runFile("/exiftool", s);
		r.flush();
		let u = X.toString();
		if (!l.success || l.exitCode !== 0) return {
			success: !1,
			data: void 0,
			error: r.getLastError() || u || "Unknown error",
			exitCode: l.exitCode
		};
		if (u && u.trim()) return {
			success: !1,
		...
		let d = i.lookup(c);
		return !d || d.type !== "file" ? {
			success: !1,
			data: void 0,
			error: `Temporary output file not found: ${c}`,
			exitCode: 0
		} : {
			success: !0,
			data: d.content instanceof Blob ? await d.content.arrayBuffer() : d.content.buffer,
			exitCode: 0
		};
	} finally {
		$(i, a);
...
//#endregion
export { re as parseMetadata, o as t, ie as writeMetadata };
