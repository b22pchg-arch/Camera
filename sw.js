...
const GSHT_CACHE = 'gsht-pwa-v124-v117-plus-exiftool-local-bundle-link';
const APP_SHELL = [
  './',
  './index.html',
  './manifest.json',
  ...
  './exifr.full.umd.js',
  './mp4box.all.min.js',
  './exiftool_wasm_tool_v124_local_bundle.html',
  './stt/vosk/vosk.js',
...
self.addEventListener('fetch', event => {
  ...
  // GSHT V124: ExifTool WASM/ZIP/DATA là tài nguyên tùy chọn rất nhạy cảm.
    ...
    event.respondWith(
      fetch(event.request, { cache: 'no-store' })
        .then(response => {
          ...
          caches.open(GSHT_CACHE).then(cache => cache.put('./exiftool_wasm_tool_v124_local_bundle.html', copy)).catch(() => {});
          return response;
        })
        .catch(() => caches.match('./exiftool_wasm_tool_v124_local_bundle.html').then(r => r || caches.match('./offline.html')))
    );
    return;
  }
...
// GSHT V124: standalone ExifTool fetch guard; binary assets pass-through only, never return text/html fallback.
